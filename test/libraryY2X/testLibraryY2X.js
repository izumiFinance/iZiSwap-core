...
describe("swap", function () {
    ...
    it("1.1 startHasY, result liquidityX > 0, rightPt > cp + 1", async function () {
        ...
        const costYAtCp = l2y('21111', cp, '1.0001', true);
        const costYAtCpWithFee = amountAddFee(costYAtCp);
        const expectResAtCp = y2xAtLiquidity(cp, '1.0001', costYAtCp, '30000', '28000');

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, costYAtCpWithFee, cp+1);
        ...
        expect(state2.liquidityX).to.equal('6889');
    });  

    it("1.2 startHasY, result liquidityX == 0, costY >= amountY, rightPt > cp + 1", async function () {
    ...
    it("1.3.2.1 startHasY, result liquidityX == 0, costY < amountY, rightPt = cp + 1", async function () {
        ...
        const costYAtRightPt = l2y('12000', rightPt, '1.0001', true);
        const expectResAtRightPt = y2xAtLiquidity(rightPt, '1.0001', costYAtRightPt, '50000', '50000');
        const costYAtRightPtWithFee = amountAddFee(costYAtRightPt);

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, getSum([costY_M1001_M50_WithFee, costY_M50_1000_WithFee, costYAtRightPtWithFee]), rightPt + 1);
    ...
    it("1.3.2.2 startHasY, result liquidityX == 0, costY < amountY, locPt = cp + 1", async function () {
        ...
        const costY_M1000_M999_NotEnough = stringMinus(yInRange('30000', -1000, -999, '1.0001', true), '1');

        const expectResAtM1000 = y2xAtLiquidity(-1000, '1.0001', costY_M1000_M999_NotEnough, '30000', '30000');
        const amountYCpM1000_WithFee = amountAddFee(getSum([costYAtCp, costY_M1000_M999_NotEnough]));
        const costYCpM1000_WithFee = amountAddFee(getSum([costYAtCp, expectResAtM1000.costY]));

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, amountYCpM1000_WithFee, rightPt);
    ...
    it("1.3.2.3 startHasY, result liquidityX == 0, costY < amountY, locPt > cp + 1", async function () {
        ...
        const costYAt212 = l2y('11000', 212, '1.0001', true);
        const expectResAt212 = y2xAtLiquidity(212, '1.0001', costYAt212, '30000', '30000');
        const costY_M50_213_WithFee = amountAddFee(getSum([costY_M50_212, costYAt212]));

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, getSum([costY_M1001_M50_WithFee, costY_M50_213_WithFee]), 10000);
    ...
    it("1.3.2.3.1 startHasY, result liquidityX == 0, costY < amountY, locPt > cp + 1", async function () {
        ...
        const costYAtM90 = l2y('11000', -90, '1.0001', true);
        const expectResAtM90 = y2xAtLiquidity(-90, '1.0001', costYAtM90, '30000', '30000');
        const costY_M1001_M89_WithFee = amountAddFee(getSum([costYAtCp, costY_M1000_M90, costYAtM90]));

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, costY_M1001_M89_WithFee, 10000);
    ...
    it("1.3.2.4 startHasY, result liquidityX == 0, costY < amountY, locPt = rightPt - 1", async function () {
        ...
        const costY_M1001_M51_WithFee = amountAddFee(getSum([costYAtCp, costY_M1000_M51]));

        const costY_M1000_M50_NotEnough = stringMinus(yInRange('30000', -1000, -50, '1.0001', true), '1');

        const amountYAtM51 = stringMinus(costY_M1000_M50_NotEnough, costY_M1000_M51);
        const expectResAtM51 = y2xAtLiquidity(-51, '1.0001', amountYAtM51, '30000', '30000');
        const amountY_M1001_M50_WithFee = amountAddFee(getSum([costYAtCp, costY_M1000_M51, amountYAtM51]));
        const costY_M1001_M50_WithFee = amountAddFee(getSum([costYAtCp, costY_M1000_M51, expectResAtM51.costY]));

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, amountY_M1001_M50_WithFee, 100000);
    ...
    it("2.1 !startHasY, result liquidityX == 0, costY < amountY, rightPt = cp + 1, complete range, finish", async function () {
        ...
        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, costY_M1001_M1000_WithFee, 100000);
    ...
    it("2.1.1 !startHasY, result liquidityX == 0, costY < amountY, rightPt > cp + 1, complete range, finish", async function () {
        ...
        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, costY_M1001_M50_WithFee, 100000);
        expect(swapResAtCp.costY).to.equal(costY_M1001_M50_WithFee);
    ...
    it("2.2 !startHasY, result liquidityX == 0, costY < amountY, locPt = cp", async function () {
        ...
        const costYAtM1001 = l2y('9999', -1001, '1.0001', true);
        const expectResAtM1001 = y2xAtLiquidity(-1001, '1.0001', costYAtM1001, '30000', '30000');
        const costYAtM1001_WithFee = amountAddFee(costYAtM1001);

        const swapResAtCp = await swapY2X(testSwap, trader, tokenX, tokenY, 3000, costYAtM1001_WithFee, rightPt);
        ...
        expect(state2.liquidity).to.equal('30000');
        expect(state2.liquidityX).to.equal('20001');
    }); 
    ...
    it("2.3 !startHasY, result liquidityX == 0, costY < amountY, locPt > cp", async function () {
        ...
        const costYAtM90 = l2y('12000', -90, '1.0001', true);
        const expectResAtM90 = y2xAtLiquidity(-90, '1.0001', costYAtM90, '30000', '30000');
        const costY_M1001_M89_WithFee = amountAddFee(getSum([costY_M1001_M90, costYAtM90]));
    ...
    it("2.4 startHasY, result liquidityX == 0, costY < amountY, locPt = rightPt - 1", async function () {
        ...
        const costY_M1001_M50_NotEnough = stringMinus(yInRange('30000', -1001, -50, '1.0001', true), '1');

        const amountYAtM51 = stringMinus(costY_M1001_M50_NotEnough, costY_M1001_M51);
        const expectResAtM51 = y2xAtLiquidity(-51, '1.0001', amountYAtM51, '30000', '30000');
        const amountY_M1001_M50_WithFee = amountAddFee(getSum([costY_M1001_M51, amountYAtM51]));
